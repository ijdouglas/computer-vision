  ...
  final_results = mclapply(results, mc.cores = 10, FUN= function(r) {
	...
	# code derived variables based on results and original data
        mutate(prediction = round(JA_prob),
	       any_handling_child = (cstream_inhand_left_hand_obj_all_child %in% 1:11) |
		 (cstream_inhand_right_hand_obj_all_child %in% 1:11),
	       any_handling_parent = (cstream_inhand_left_hand_obj_all_parent %in% 1:11) |
		 (cstream_inhand_right_hand_obj_all_parent %in% 1:11),
	       both_handling = (any_handling_child) & (any_handling_parent),
	       child_only_handling = (any_handling_child) & (!any_handling_parent),
	       parent_only_handling = (!any_handling_child) & (any_handling_parent),
	       any_handling_anyone = (any_handling_child) | (any_handling_parent),
	       parent_eye_child_face = cstream_eye_roi_fixation_parent==11,
	       child_eye_parent_face = cstream_eye_roi_fixation_child == 11)
  })
