...
fm = fm %>%
	...
	mutate(path = paste0('experiment_', experiment, '/included/', kidID, '/cam07_frames_p/', image)) %>%
...
mclapply(X=unique(fm$subject), mc.cores = 10, FUN=function(id) {
    ...
    write.csv(train, paste0('../data/child/train/', id, '_train.csv'), row.names=F)
    write.csv(test, paste0('../data/child/test/', id, '_test.csv'), row.names=F)
})
